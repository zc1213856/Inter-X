...
class SMPLModel(Module):
    def __init__(self, device=None, model_path='./SMPL_NEUTRAL.pkl',
        ...
        self.J_halpe_regressor = torch.from_numpy(np.load('data/body_models/smpl/J_regressor_halpe.npy')).to(dtype=self.data_type, device=self.device)
        ...
        self.faces = params['f']
        self.root = torch.matmul(self.J_regressor, self.v_template)[0]
